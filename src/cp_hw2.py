...
def read_colorchecker_gm():
    ...
    RGB = apply_color_matrix(
        XYZ,
        M_XYZ_to_RGB)  # we want linear RGB values for our HDR measurements
...
def XYZ_to_RGB_linear(rgb_space='sRGB'):
    ...
    reference_whites = [
        'D65', 'D65', 'D50', 'D50', 'D65', 'E', 'D50', 'D50', 'D50', 'D50',
        'C', 'D65', 'D50', 'D65', 'D65', 'D50'
    ]
    ...
    S = np.linalg.inv(np.array([[Xr, Xg, Xb], [Yr, Yg, Yb], [Zr, Zg, Zb]
                                ])) @ np.array([Xw, Yw, Zw])

    # this matrix is RGB to XYZ
    M = np.array([[S[0] * Xr, S[1] * Xg, S[2] * Xb],
